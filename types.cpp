...
//      we build the ty_name first with a placeholder (such as ty_error) and then fill it in later:
	...
	//
	EM_condition(type_of_list->kind != Ty_name, "an error in type examples");
...
static string to_String_rec(Ty_ty t, Ty_tyList &types_done);
static string to_String_rec(Ty_fieldList fl, Ty_tyList &types_done);
static string to_String_rec(Ty_tyList tl, Ty_tyList &types_done);
...
static string to_String_rec(Ty_ty t, Ty_tyList &types_done)
{
  EM_condition(t == 0 || t->kind < 0 || to_String(t->kind) != INVALID_TYPE_KIND, "bad type given in to_String_rec()" );
  	string result = to_String(t->kind) + type_address(t);

      bool repeated = false;
      for (Ty_tyList ls = types_done; ls; ls = ls->tail){
          if (ls->head == t)
              repeated = true;
              break;
      }
	...
	types_done = Ty_TyList(t, types_done);
