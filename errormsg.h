...
#include "cmake-build-debug/location.hh" // auto-built by bison, when used with the location option turned on
...
// subtypes of EM_error that only error on a certain condition
void EM_condition (bool condition, string message = "condition failed", bool fatal= false, Position position = Position::undefined());
